...
public final class NumiEngine {
    ...
    nonisolated public static let ratesUpdatedNotification = Notification.Name("vektor.engine.ratesUpdated")

    private static let logger = Logger(subsystem: "app.vektor.Vektor", category: "engine")
    ...
    public func evaluate(_ source: String) -> [LineResult] {
        ...
        _ = context.evaluateScript("vektor.resetScope();")
        ...
        for (idx, raw) in lines.enumerated() {
            ...
            let precision = max(2, min(14, UserDefaults.standard.object(forKey: "vektor.precision") as? Int ?? 14))
            let js = """
            (() => { try {
                const v = vektor.evalLine('\(escaped)');
                return vektor.format(v, \(precision));
    ...
    private func handleTimezoneLine(_ line: String) -> String? {
        ...
        // preprocessor.
        if let r = line.range(of: #"\s+(?:to|in|as)\s+(?:hours?|minutes?|seconds?)\s*$"#,
    ...
    /// register a placeholder rate if none is loaded yet. `vektor.ensureCurrency`
    ...
    private func ensureCurrencies(in line: String) {
        ...
        for m in matches {
            ...
            if seen.insert(code).inserted {
                _ = context.evaluateScript("vektor.ensureCurrency('\(code)');")
            }
        }
    }
